...
module.exports.execute = async(client, message, args,ayar) => {
   ...
   // if(!message.member.roles.cache.array().some(rol => message.guild.roles.cache.get(ayar.staffrole).rawPosition <= rol.rawPosition)) return  message.reply("`Bu komut yetkililere özeldir.`");
    const embed = new MessageEmbed().setColor("RANDOM").setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true, size: 2048}));
    MemberStats.find({guildID: message.guild.id}).exec((err, data) => {
		...
		embed.addField('Mesaj Sıralaması', toplamChatSiralama);
		embed.addField('Ses Sıralaması', toplamSesSiralama);
        embed.addField('1 Haftalık Ses Sıralaması', haftalikSesSiralama);
        embed.addField('1 Haftalık Mesaj Sıralaması', haftalikChatSiralama);
