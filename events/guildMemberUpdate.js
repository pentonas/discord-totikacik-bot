            ...
            newMember.roles.cache.forEach(async role => {
                if (!oldMember.roles.cache.has(role.id)) {
                    const emed = new Discord.MessageEmbed()
                        ...
                        .setColor("RANDOM")
                        .setDescription(`<a:immortal_unlem:829008274920964116> ${hedef} - (\`${hedef.id}\`) kişisine rol eklendi!`)
                        ...
                        .setTimestamp()
                    this.client.channels.cache.get("832545686783787048").send(emed)
                    roller.findOne({
                        user: hedef.id
                    }, async (err, res) => {
                        if (!res) {
            ...
            oldMember.roles.cache.forEach(async role => {
                if (!newMember.roles.cache.has(role.id)) {
                    const emeed = new Discord.MessageEmbed()
                        .setAuthor(hedef.tag, hedef.displayAvatarURL({ dynamic: true }))
                        .setColor("RANDOM")
                        .setDescription(`<a:immortal_unlem:829008274920964116> ${hedef} - (\`${hedef.id}\`) kişisinden rol alındı!`)
                        .addField("Alan Kişi", `${yapan} - (\`${yapan.id}\`)`, false)
                        .addField("Alınan Rol", `${role}`, false)
                        .setFooter(yapan.tag, yapan.displayAvatarURL({ dynamic: true }))
                        .setTimestamp()
                    this.client.channels.cache.get("832545686783787048").send(emeed)
                    roller.findOne({
                        user: hedef.id
                    }, async (err, res) => {
                        if (!res) {
