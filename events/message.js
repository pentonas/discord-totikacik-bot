...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.content == "!tag" || message.content == ".tag") {
      message.channel.send(clientayar.tag)
    }
    if (message.content == "!link") {
      message.channel.send("https://discord.gg/qWgcKMY2RB <@" + message.author.id + ">")
    }
    ...
    if (message.channel.id == clientayar.genelchat) {
    ...
    if (!owners.includes(message.author.id)) {
      ...
      if (blockArr.length == 9) {
        ...
        this.client.channels.cache.get(clientayar.komutengellog).send(`**${message.author.tag}** - ${message.author}(\`${message.author.id}\`) komut engeli yedi.Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
    ...
    //this.client.channels.cache.get(clientayar.komutlog).send(`${message.author.tag} (${message.author.id}) komut kullandı "${cmd.help.name}" kullandığı kanal ${message.channel.name}`)
