...
class Booster extends Command {
    ...
    async run(message, args, data) {
        if(!message.member.roles.cache.has(clientayar.booser)) return
        if(!message.member.roles.cache.has(clientayar.booser)) return
      ...
      if(args.slice(0).join(" ").length > 32) return
      message.author.username.includes(clientayar.tag) ? await message.member.setNickname(`${clientayar.tag} ${args.slice(0).join(" ")}`) : await message.member.setNickname(`${clientayar.tag2} ${args.slice(0).join(" ")}`)
      await message.react(this.client.yes)
    }
}
