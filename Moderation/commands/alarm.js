...
class Alarm extends Command {
    ...
    async run(message, args, perm) {
        await data.findOne({ user: message.author.id }, async (err, res) => {
            let time = args[0]
            if (!time || isNaN(ms(time))) return message.channel.send("Alarm kurmam için bir zaman belirtmelisin.")
            ...
            let regexpxs = /h?t?t?p?s?:?\/?\/?discord.?gg\/?[a-zA-Z0-9]+/gi
            let regexSecond = /h?t?t?p?s?:?\/?\/?discorda?p?p?.?com\/?invites\/?[a-zA-Z0-9]+/
           // if(regexpxs.exec(message.content)) return message.channel.send(`Cidden reklamını mı hatırlamak için alarm kurdun?`);
            ...
            message.channel.send(":alarm_clock: | Alarmı kurdum sana bunu " + tamam + " hatırlatacağım.")
        })
    }
}
