...
class GoLive extends Command {
    ...
    async run(message, args, perm) {
        ...
        if(!user.roles.cache.has(clientayar.golive)) {
            await this.client.yolla(`${user} kişisine <@&${clientayar.golive}> rolü verildi.`, message.author, message.channel)
            user.roles.add(clientayar.golive)
        } else{
            await this.client.yolla(`${user} kişisine <@&${clientayar.golive}> rolü alındı.`, message.author, message.channel)
            user.roles.remove(clientayar.golive)
        }
    }
}
