...
class Yetkilisay extends Command {
    ...
    async run(message, args, level) {
        if (!message.member.hasPermission("ADMINISTRATOR")) return
            let roles = args.length > 0 ? message.mentions.roles.first() || message.guild.roles.cache.get(args[0]) : message.guild.roles.cache.find(x => x.id == "832517709828325378")
            ...
            message.channel.send("Online olup seste olmayan <@&"+roles+"> rolündeki yetkili sayısı: " + üyeler.size + "")
