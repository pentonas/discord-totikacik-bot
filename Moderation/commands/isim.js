...
class İsim extends Command {
    ...
    async run(message, args, level) {
        ...
        isimler.findOne({ user: member.id }, async (err, res) => {
            if (!res) {
                member.user.username.includes(clientayar.tag) ? await member.setNickname(`${clientayar.tag} ${isim} | ${yaş}`) : await member.setNickname(`${clientayar.tag2} ${isim} | ${yaş}`)
                ...
                if (message.channel.id === clientayar.registerchat) {
