    ...
    async run(message, args, level) {
        ...
        await kayıtlar.findOne({ user: message.author.id }, async (err, res) => {
            ...
            if (!user.roles.cache.has(clientayar.registerhammer)) {
                ...
                await user.roles.add([clientayar.erkekrol1, clientayar.erkekrol2])
                await user.roles.remove([clientayar.kayıtsız])
                const emux = new Discord.MessageEmbed()
                    .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                    .setDescription(`${user} kullanıcısı başarıyla <@&${clientayar.erkekrol1}> olarak kayıt edildi.`)
                    .setColor("RANDOM")
            ...
            isimler.findOne({ user: user.id }, async (err, res) => {
                ...
                if (!res) {
                    let arr = []
                    arr.push({ isim: `${isim} | ${yaş}`, state: `<@&${clientayar.erkekrol1}>`, yetkili: message.author.id })
                    ...
                    res.isimler.push({ isim: `${isim} | ${yaş}`, state: `<@&${clientayar.erkekrol1}>`, yetkili: message.author.id })
                    res.save().catch(e => console.log(e))
                }
            })
        })
