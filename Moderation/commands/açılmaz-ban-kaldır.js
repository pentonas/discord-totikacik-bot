...
class NoUNBANRemove extends Command {
    constructor(client) {
        super(client, {
            name: "infaz-kaldır",
            aliases: ["acılmazban-kaldır", "acılmaz-ban-kaldır", "açılmaz-ban-kaldır", "açılamazban-kaldır", "acılamazban-kaldır", "açılamaz-ban-kaldır", "acılamaz-ban-kaldır", "infazkaldır", "infaz-kaldır"]
        });
    }
                    ...
                    if (!ban) {
                        ...
                        await db.findOne({ user: res.id }, async (err, doc) => {
                            if (!doc) {
                                ...
                                if (!message.member.roles.cache.has(ayarlar.banhammer) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed)
