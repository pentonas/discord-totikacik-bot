...
class İnfo extends Command {
    ...
    async run(message, args, perm) {
        ...
        await cezalar.findOne({ user: user.id }, async (err, doc) => {
            if (!doc) {
                cezalı = "Veritabanında cezalı bilgisi bulunmamaktadır."
            } else {
                if (doc.ceza == false) {
                    cezalı = "Veritabanında chat mute bilgisi bulunmamaktadır."
        ...
        await mutes.findOne({ user: user.id }, async (err, doc) => {
            if (!doc) {
                mute = "Veritabanında chat mute bilgisi bulunmamaktadır."
            } else {
                if (doc.muted == false) {
                    mute = "Veritabanında chat mute bilgisi bulunmamaktadır."
