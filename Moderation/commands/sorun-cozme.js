...
class SorunCozme extends Command {
    ...
    async run(message, args, perm) {
        ...
        if(!user.roles.cache.has(clientayar.soruncozucu)) {
            await this.client.yolla(`${user} kişisine <@&${clientayar.soruncozucu}> rolü verildi.`, message.author, message.channel)
            user.roles.add(clientayar.soruncozucu)
        } else{
            await this.client.yolla(`${user} kişisine <@&${clientayar.soruncozucu}> rolü alındı.`, message.author, message.channel)
            user.roles.remove(clientayar.soruncozucu)
        }
    }
}
