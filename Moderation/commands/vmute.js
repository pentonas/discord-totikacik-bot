...
class VMute extends Command {
   ...
   //     let cıkaralım = time + Date.parse(new Date().toLocaleString("tr-TR", { timeZone: "Asia/Istanbul" }));
         ...
         let sonraki = this.client.toDate(Date.now() + time)
        if(user.voice.channel) {
        user.voice.setMute(true)
        await message.channel.send(`${this.client.ok} <@${user.id}> kişisi ${await this.client.turkishDate(time)} boyunca ses kanallarında susturuldu. (Ceza Numarası: \`#${count}\`)`)
        ...
        await wmute.findOne({user: user.id}, async(err,res) => {
            if(!res) {
                await message.channel.send(`${this.client.no} <@${user.id}> kişisinin ${await this.client.turkishDate(time)} sürelik ses mutesi başlatılamadı kullanıcı sese bağlanınca otomatik olarak cezası başlayacak. (Ceza Numarası: \`#${count}\`)`)
            ...
            await data.find({}).sort({ ihlal: "descending" }).exec(async (err, res) => {
                const newData = new data({
                    ...
                    bitiş: this.client.toDate(Date.now() + time)
                })
